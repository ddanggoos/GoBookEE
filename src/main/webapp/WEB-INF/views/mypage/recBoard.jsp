...
<main class="container py-4" style="max-width: 600px;">
	<div class="d-flex align-items-center mb-4">
		...
		<h5 class="fw-bold mb-0">내가 추천한 글</h5>
	</div>
	...
	<div class="search-tab">
		...
		<button type="button" class="tab-btn" data-tab="study">스터디</button>
...
function loadMyPosts(tab, cPage = 1) {
    $.ajax({
        url: "<%=request.getContextPath()%>/recboard/ajax",
        method: "GET",
        data: {
            tab: tab,
            cPage: cPage
        ...
        success: function (res) {
            ...
            if (!res.list || res.list.length === 0) {
                container.append("<p class='text-muted text-center'>추천한 글이 없습니다.</p>");
                return;
            }
            ...
            res.list.forEach(item => {
                ...
                if (tab === "review") {
                    ...
                    html =  `
                        ...
                        onclick="location.assign('<%=request.getContextPath()%>/study/view?seq=\${item.studySeq}')">
                       <div class="row g-0">
                       <div class="col-4 d-flex align-items-center justify-content-center p-3">
                         <img src="<%=request.getContextPath()%>/upload/photo/\${item.photoRenamedName}"
