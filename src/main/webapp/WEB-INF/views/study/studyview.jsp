...
<main>
	...
	<div>
		...
		<small><%= studyview.getUserNickName() %></small>
...
%>
						<div class="d-grid gap-2">
							<button class="btn-outline-danger btn" style="border-radius: 30px; margin-bottom: 10px;" id="study-application">거절됨</button>
						</div>
<%
					} else if("N".equals(confirm)) {
%>
						<div class="d-grid gap-2">
							<button class="btn-outline-success btn" style="border-radius: 30px; margin-bottom: 10px;" id="study-application">신청완료</button>
						</div>
<%
					}
					break;
			...
			if (!found) {
				...
				<!-- 신청 버튼 (모달 트리거) -->
				<div class="d-grid gap-2">
				  <button type="button" class="btn btn-outline-dark" style="border-radius: 30px; margin-bottom: 10px;" data-bs-toggle="modal" data-bs-target="#applyModal">
				    신청하기
				  </button>
				</div>
