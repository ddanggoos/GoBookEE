...
@WebServlet("/studygroup/studygroupendlist")
public class StudyGroupEndListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private StudyGroupService service = StudyGroupService.studyGroupService();
    public StudyGroupEndListServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		AjaxPageBarTemplate pb = new AjaxPageBarTemplate(cPage, numPerPage, service.allStudyGroupsCountEnd(userseq));
		
		List<StudyList> list;
		if("recommend".equals(sort)) {
			list=StudyGroupService.studyGroupService().getGroupStudiesByRecEnd(userseq, cPage, numPerPage);
		}else {
